...
async def add_new_playbook(
    ...
    try:
        ...
        existing = catalog_service.read_catalog_raw()
        ...
        if file_content:
            saved_filename = execution_service.save_playbook_file(filename, file_content)
        else:
        ...
        if not catalog_service.save_catalog(existing):
            if saved_filename and file_content:
                execution_service.delete_playbook_file(saved_filename)
        ...
        if catalog_updated and saved_filename:
            catalog_service.remove_catalog_entry(saved_filename)
        if mongo_inserted:
            pass
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        try:
            ...
            new_doc = {
                ...
                "file_path": str(execution_service.PLAYBOOKS_DIR / filename),
                "last_executed": "Never Executed",
...
async def delete_playbook_entry(playbook_id: str):
    ...
    try:
        ...
        if filename:
            catalog_service.remove_catalog_entry(filename)
            execution_service.delete_playbook_file(filename)
