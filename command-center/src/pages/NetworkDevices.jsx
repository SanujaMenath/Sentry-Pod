...
const DeviceCard = ({ device }) => {
  ...
  return (
    <div className="bg-[#1D293DED] border border-slate-700/50 rounded-3xl p-6 
      flex flex-col h-full transition-all duration-300 ease-out hover:border-blue-500/50 hover:-translate-y-0.5 hover:shadow-[0_20px_40px_rgba(0,0,0,0.4),0_0_15px_rgba(59,130,246,0.1)]
      cursor-pointer">
      {/* Header */}
      ...
      <div className="grid grid-cols-3 gap-2 mb-6">
        {[
          { label: 'Model', val: device.model.split(' ')[2] || device.model },
          { label: 'OS', val: device.version },
          { label: 'Uptime', val: device.uptime }
        ].map((item, i) => (
          <div key={i} className="bg-[#0d1117]/50 p-2 rounded-xl border border-slate-800/50 text-center">
            <p className="text-[9px] uppercase text-slate-600 font-bold mb-0.5">{item.label}</p>
            <p className="text-[11px] text-slate-300 font-medium truncate">{item.val}</p>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="p-8 space-y-8 font-sans min-h-screen bg-linear-to-br from-[#F8FAFC] to-[#D1D5DB]">
      ...
      <div className="flex items-start justify-between">
        ...
        <button className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-blue-600/20 active:scale-95 transition-all">
          <PlusCircle size={18} />
          Add Device
        </button>
      </div>
